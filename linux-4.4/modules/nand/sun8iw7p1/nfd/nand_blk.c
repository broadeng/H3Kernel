		      ...
		      unsigned long arg)
{
	...
	if (cmd != SECBLK_READ && cmd != SECBLK_WRITE) {
		if (copy_from_user(&burn_param, (struct burn_param_t __user *)arg, sizeof(burn_param))) {
			nand_dbg_err("nand_ioctl input arg err,cmd=0x%x.\n", cmd);
			return -EFAULT;
		}
	} else 
		nand_dbg_err("SST reading or writing\n");
	...
	case SECBLK_READ:
		...
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&sec_op, (struct secblc_op_t *)arg, sizeof(struct secblc_op_t))) {
			nand_dbg_err("copy user buffer failed.\n");
			return -EFAULT;
		}
