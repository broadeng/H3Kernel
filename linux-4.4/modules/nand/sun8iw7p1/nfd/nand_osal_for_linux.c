...
void NAND_GetVccq(void)
{
	...
	reg_val = readl((void __iomem *)PL_PUL0_REG);
	reg_val &= ~(1 << 7);
	reg_val |= (1 << 6);
	writel(reg_val, (void __iomem *)PL_PUL0_REG);

	reg_val = readl((void __iomem *)PL_CF0_REG);
	reg_val &= ~(6 << 12);
	reg_val |= (1 << 12);
	writel(reg_val, (void __iomem *)PL_CF0_REG);

	reg_val = readl((void __iomem *)PL_DAT_REG);
	reg_val |= (1 << 3);
	writel(reg_val, (void __iomem *)PL_DAT_REG);
