...
build_check()
{
    ...
    if [ "x$SUNXI_SPARSE_CHECK" = "x1" ] && [ -f ../tools/codecheck/sparse/sparse ];then
        ...
        make CHECK=${CHECK} ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} -j${LICHEE_JLEVEL} \
    ...
    if [ "x$SUNXI_STACK_CHECK" = "x1" ];then
        make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} -j${LICHEE_JLEVEL} checkstack 2>&1 \
...
build_kernel()
{
    ...
    if [ "${ARCH}" = "arm" ]; then
        ...
        cp -f rootfs_64bit.cpio.gz output/rootfs.cpio.gz
    fi
	...
	if [ -e ${LICHEE_MOD_DIR}/$(basename $file) ]; then
		${STRIP} -d ${LICHEE_MOD_DIR}/$(basename $file) >/dev/null
	fi
    done
...
build_modules()
{
    ...
    local rebuild_kernel=1;
    local LICHEE_NAND_DIR="${LICHEE_KDIR}/modules/nand/sun8iw7p1";

    cd ${LICHEE_KDIR}
    cp rootfs.cpio.xz ./output
    cd output && mkdir ramfs && cd ramfs && cp ../rootfs.cpio.xz ./
    xz -d --check=crc32 --lzma2=dict=512KiB rootfs.cpio.xz
    cpio -idmv < rootfs.cpio

    if [ -a ${LICHEE_NAND_DIR}/nand.ko ]; then
        if [ -a ${LICHEE_KDIR}/output/ramfs/lib/modules/${KERNEL_VERSION}/nand.ko ]; then
            rebuild_kernel=0;
        fi
    fi
    cd ${LICHEE_KDIR}
    ...
    if  [ ${rebuild_kernel} == 1 ]; then
        cd ${LICHEE_KDIR}/output/ramfs && mkdir -p ./lib/modules/${KERNEL_VERSION}
	    if [ -e ${LICHEE_MOD_DIR}/nand.ko ]; then
		    cp ${LICHEE_MOD_DIR}/nand.ko ./lib/modules/${KERNEL_VERSION}
		    if [ $? -ne 0 ]; then
        ...
        xz --check=crc32 --lzma2=dict=512KiB rootfs.cpio
        cp rootfs.cpio.xz ${LICHEE_KDIR}
        cd ${LICHEE_KDIR}
        build_kernel
    fi
}
