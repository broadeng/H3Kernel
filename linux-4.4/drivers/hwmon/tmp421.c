		...
		struct device_attribute *devattr, char *buf)
{
	...
	id_value = (i2c_smbus_read_byte_data(client, TMP421_MANUFACTURER_ID_REG)
			<< 8) | i2c_smbus_read_byte_data(client, TMP421_DEVICE_ID_REG);
		...
		struct device_attribute *devattr, char *buf)
{
	...
	return sprintf(buf,
			"MID=%02x DID=%02x STATUS=%02x CONFIG_REG_1=%02x CONVERSION_RATE_REG=%02x\n",
