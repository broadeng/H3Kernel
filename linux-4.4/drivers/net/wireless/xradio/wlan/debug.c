...
/* save softwave. */
#ifdef XRADIO_SAVE_SOFTWAVE
debug_save_items DbgSaveItems;

void debug_item_init(void)
{
	memset(&DbgSaveItems, 0, sizeof(DbgSaveItems));
}

void debug_save_item(struct xradio_common *hw_priv, debug_save_items *dbg_items, u32 item_num, ...)
{
	int i;
	va_list args;
	dbg_items->total_cnt++;
	if ((hw_priv->debug->wavectrl_type == 0) ||
		((hw_priv->debug->wavectrl_type == 1) && (dbg_items->total_cnt > hw_priv->debug->wavectrl_offset) && (dbg_items->wr_depth < DEBUG_SAVE_ITEM_DEPTH))) {
		va_start(args, item_num);
		BUG_ON(item_num > DEBUG_SAVE_ITEM_NUM);
		dbg_items->item_num = item_num;

		for (i = 0; i < item_num; i++) {
			dbg_items->save_item[i][dbg_items->wr_idx] = va_arg(args, u32);
		}
		/* update status */
		dbg_items->wr_idx = (dbg_items->wr_idx+1) % DEBUG_SAVE_ITEM_DEPTH;
		if (dbg_items->wr_depth < DEBUG_SAVE_ITEM_DEPTH)
			dbg_items->wr_depth++;
...
/*
 *wavectrl_type default 0, always update. 1: just record once at offset after "wavectrl_type=1 write" trigger.
 *wavectrl_offset valid wavectrl_type==1, record wave after 'offset' clock
 */
static ssize_t xradio_wavectrl_read(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	char buf[60];
	...
	sprintf(buf, "wavectrl_type/offset: %d,%d\n", hw_priv->debug->wavectrl_type, hw_priv->debug->wavectrl_offset);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_wavectrl_write(struct file *file,
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	char buf[80] = {0};
	...
	if (copy_from_user(buf, user_buf, count))
		return -EFAULT;
	hw_priv->debug->wavectrl_type = (simple_strtoul(start, &end, 10));
	printk(KERN_ERR "wavectrl_type = %d", hw_priv->debug->wavectrl_type);
	start = end + 1;
	hw_priv->debug->wavectrl_offset = simple_strtoul(start, &end, 10);
	printk(KERN_ERR "wavectrl_offset = %d", hw_priv->debug->wavectrl_offset);

	if (hw_priv->debug->wavectrl_type == 1) {
		debug_item_init();
		next_rx_cnt  = 0;
		rx_total_cnt = 0;
		tx_total_cnt = 0;
...
static const struct file_operations fops_wavectrl = {
	.open = xradio_generic_open,
	.write = xradio_wavectrl_write,
	.read = xradio_wavectrl_read,
	.llseek = default_llseek,
};

static int xradio_softwave_show(struct seq_file *seq, void *v)
{
	int i, index;
	debug_save_items *dbg_items = &DbgSaveItems;
	seq_printf(seq, "rxtxzzz\trx_cnt\tnxtrx\ttx_cnt\trxburst\ttxcfm\tdur_us\tbh_tx\tqueued\tq_pending\n");
	for (i = 0; i < dbg_items->wr_depth; i++) {
		index = (dbg_items->wr_idx + i) % DEBUG_SAVE_ITEM_DEPTH;
		seq_printf(seq, "%4u\t%4u\t%4u\t%4u\t%4u\t%4u\t%4u\t%4u\t%4u\t%4u\n",
						dbg_items->save_item[0][index],
						dbg_items->save_item[1][index],
						dbg_items->save_item[2][index],
						dbg_items->save_item[3][index],
						dbg_items->save_item[4][index],
						dbg_items->save_item[5][index],
						dbg_items->save_item[6][index],
						dbg_items->save_item[7][index],
						dbg_items->save_item[8][index],
						dbg_items->save_item[9][index]);
	}
	return 0;
}
static int xradio_softwave_open(struct inode *inode, struct file *file)
{
	return single_open(file, &xradio_softwave_show, inode->i_private);
}
static const struct file_operations fops_softwave = {
	.open = xradio_softwave_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.owner = THIS_MODULE,
...
#endif
int xradio_host_dbg_init(struct xradio_common *hw_priv)
{
	int line = 0;
	xradio_dbg(XRADIO_DBG_TRC, "%s\n", __func__);
...
#define ERR_LINE  do { line = __LINE__; goto err; } while (0)
	...
	debugfs_host = debugfs_create_dir("xradio_host_dbg",
...
#define ERR_LINE  do { line = __LINE__; goto err; } while (0)
	...
	ret = xradio_host_dbg_init(hw_priv);
	if (ret) {
		xradio_dbg(XRADIO_DBG_ERROR, "%s host dbg init failed(%d)!\n",
						__func__, ret);
		return ret;
	}
...
#ifdef XRADIO_SAVE_SOFTWAVE
	if (!debugfs_create_file("wavectrl", S_IRUSR | S_IWUSR,
		d->debugfs_phy, hw_priv, &fops_wavectrl))
		ERR_LINE;

	if (!debugfs_create_file("softwave", S_IRUSR, d->debugfs_phy,
			hw_priv, &fops_softwave))
		ERR_LINE;
#endif
	...
	if (debugfs_host && !debugfs_hwinfo) {
		debugfs_hwinfo = debugfs_create_file("hwinfo", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH,
