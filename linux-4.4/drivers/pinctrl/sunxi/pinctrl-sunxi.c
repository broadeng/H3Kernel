		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(str_buf, user_buf, count))
	  	return -EFAULT;

	err = sscanf(str_buf, "%s %u %u %u %u", sunxi_dbg_pinname,
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	err = sscanf(str_buf, "%19s", sunxi_dbg_pinname);
	kfree(str_buf);
	if (err != 1)
		return -EINVAL;
		...
		size_t count, loff_t *ppos)
{
	...
	err = sscanf(str_buf, "%s %lu", sunxi_dbg_pinname, &function);
	kfree(str_buf);
	if (err != 2)
		return err;
		...
		size_t count, loff_t *ppos)
{
	...
	err = sscanf(str_buf, "%s %lu", sunxi_dbg_pinname, &data);
	kfree(str_buf);
	if (err != 2)
		return err;
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	err = sscanf(str_buf, "%s %lu", sunxi_dbg_pinname, &dlevel);
	kfree(str_buf);
	if (err != 2)
		return err;
		...
		size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(str_buf, user_buf, count))
	  	return -EFAULT;
	err = sscanf(str_buf, "%s %lu", sunxi_dbg_pinname, &pull);
	kfree(str_buf);
	if (err != 2)
		return err;
