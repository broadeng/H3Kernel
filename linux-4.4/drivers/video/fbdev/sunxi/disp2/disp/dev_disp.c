...
static DEVICE_ATTR(boot_para, 0660,
				 ...
				 struct disp_init_para *init_para)
{
	...
	if ((DISP_SCREEN_NUM > 1)
			&& ((value & 0xFF000000) == (init_para->output_type[1] << 24))) {
...
#ifdef CONFIG_DISP2_SUNXI_COMPOSER2
extern int composer2_init(struct disp_drv_info *p_disp_drv);
#endif
...
#endif
#if defined(CONFIG_DISP2_SUNXI_COMPOSER2)
	composer2_init(&g_disp_drv);
#endif
...
static int disp_probe(struct platform_device *pdev)
{
	...
	if (!g_disp_drv.irq_no[DISP_MOD_DE])
		dev_err(&pdev->dev,
			"irq_of_parse_and_map de irq %d fail for dsi\n", i);
	counter++;
	...
	if (!g_disp_drv.irq_no[DISP_MOD_EINK])
		dev_err(&pdev->dev,
			"irq_of_parse_and_map eink irq %d fail for dsi\n", i);
	counter++;
#endif
		...
		for (screen_id = 0; screen_id < num_screens; screen_id++) {
			...
			if (suspend_output_type[screen_id] !=
				...
				if (mgr->device->set_mode
					...
					mgr->device->set_mode(mgr->device,
							      mode);
				}
				if (!mgr->device->is_enabled(mgr->device))
					mgr->device->enable(mgr->device);
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_LAYER_SET_CONFIG:
		{
			if (copy_from_user(lyr_cfg, (void __user *)ubuffer[1],
